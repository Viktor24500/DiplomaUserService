...
namespace UserServiceTest.UserRoles
{
	...
	public class UpdateUserRoleTest
	{
		...
		public async Task UpdateUserRolesAsync_Fail_UserNegativeId()
		{
			...
			userRoleRes.Data = userRoleDTO;

			_mockUserRepository.Setup(user => user.GetUser(updateParam.UserId)).ReturnsAsync(userRes);
			_mockRoleRepository.Setup(role => role.GetRole(updateParam.RoleId)).ReturnsAsync(roleRes);
			_mockUserRoleRepository.Setup(userRole => userRole.GetUserRoleByUserRoleId(updateParam.UserRoleId)).ReturnsAsync(userRoleRes);
			_mockUserRoleRepository.Setup(userRole => userRole.GetUserRoleByUserId(updateParam.UserId)).ReturnsAsync(userRoleRes);

			// Act
			Result<UserRole> result = await _userRoleService.UpdateUserRole(updateParam);
		...
		public async Task UpdateUserRolesAsync_Fail_UserRoleNegativeId()
		{
			...
			Assert.IsNotNull(result);
			Assert.AreEqual((int)ErrorCodes.BadRequest, result.ErrorCode);
			Assert.AreEqual("userRoleId can't be negative", result.ErrorMessage);
		}
		...
		public async Task UpdateUserRolesAsync_Fail_UserRoleNotFound()
		{
			...
			Result<UserRoleDTO> userRoleRes = new Result<UserRoleDTO>();
			userRoleRes.ErrorCode = (int)ErrorCodes.NotFound;
		...
		public async Task UpdateUserRolesAsync_Success()
		{
			...
			Result updateUserRoleRes = new Result();

			_mockUserRepository.Setup(user => user.GetUser(updateParam.UserId)).ReturnsAsync(userRes);
			_mockRoleRepository.Setup(role => role.GetRole(updateParam.RoleId)).ReturnsAsync(roleRes);
			_mockUserRoleRepository.Setup(userRole => userRole.UpdateUserRole(updateParam.UserRoleId, updateParam.UserId, updateParam.RoleId)).ReturnsAsync(updateUserRoleRes);
			_mockUserRoleRepository.Setup(userRole => userRole.GetUserRoleByUserRoleId(updateParam.UserRoleId)).ReturnsAsync(userRoleRes);
			_mockUserRoleRepository.Setup(userRole => userRole.GetUserRoleByUserId(updateParam.UserId)).ReturnsAsync(userRoleRes);
