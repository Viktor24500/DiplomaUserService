...
namespace SystemUserService.DataAccess.Repositories.Intefaces
{
	public interface IUsersRepository
	{
		...
		Task<Result> UpdateUser(int id, string email, string firstName,
			string lastName, string? comments, bool isActive, string phoneNumber);
		Task<Result<int>> CreateUser(string username, string userPassword,
			string email, string firstName, string lastName, string? comments,
		...
		Task<Result<UserDTO>> GetUserByPhoneNumber(string phoneNumber);
	}
}
