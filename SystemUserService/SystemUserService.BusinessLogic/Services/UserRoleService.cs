...
namespace SystemUserService.BusinessLogic.Services
{
	public class UserRoleService : IUserRoleService
	{
		...
		public async Task<Result<List<UserRole>>> CreateUserRole(UserRoleCreateParameters userRoleCreateParam)
		{
			//Validate role id
			...
			if (repUserResult.ErrorCode == (int)ErrorCodes.NotFound)
			{
				...
				result.ErrorMessage = $"Role with id {userRoleCreateParam.UserId} not exist";
				_logger.LogError(result.ErrorMessage);
				return result;
			}
			...
			if (repRoleResult.ErrorCode == (int)ErrorCodes.NotFound)
			{
				...
				result.ErrorMessage = $"Permission with id {userRoleCreateParam.RoleId} not exist";
				_logger.LogError(result.ErrorMessage);
				return result;
			}
			...
			{
				Result<List<UserRoleDTO>> repReult = await _userRolesRepository.GetUserRoleByUserId(userRoleCreateParam.UserId);
		...
		public async Task<Result<List<UserRole>>> UpdateUserRole(UserRoleUpdateParameters userRoleUpdateParam)
		{
			Result<List<UserRole>> result = new Result<List<UserRole>>();
			...
			{
				result.ErrorCode = (int)ErrorCodes.NotFound;
				result.ErrorMessage = $"Role with id {userRoleUpdateParam.UserId} not exist";
				_logger.LogError(result.ErrorMessage);
				return result;
			}
			...
			{
				result.ErrorCode = (int)ErrorCodes.Conflict;
				result.ErrorMessage = $"Permission with id {userRoleUpdateParam.RoleId} not exist";
				_logger.LogError(result.ErrorMessage);
				return result;
			}
			...
			if (repUserRoleResult.ErrorCode == (int)ErrorCodes.Success)
			{
				Result<List<UserRoleDTO>> repReult = await _userRolesRepository.GetUserRoleByUserId(userRoleUpdateParam.UserId);
				result.Data = repReult.Data.MapToUserRoleCollection();
				return result;
			}
			return result;
